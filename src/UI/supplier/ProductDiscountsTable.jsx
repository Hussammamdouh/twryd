  ...
  const handleAction = async (type, discount) => {
    ...
    try {
      if (type === 'delete') {
        ...
        toast.show(t('product_discounts.discount_removed_successfully'), 'success');
      }
      onAction?.();
    } catch (err) {
      toast.show(err.message || t('product_discounts.action_failed'), 'error');
    } finally {
  ...
  if (!productDiscounts || productDiscounts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-gray-400 gap-4">
        <div className="text-center">{t('product_discounts.no_product_discounts_found')}</div>
        <button
          className="theme-button px-4 py-2 rounded-lg font-bold shadow w-full sm:w-auto"
          onClick={onAdd}
        >
          {t('product_discounts.add_new_discount')}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="min-w-full text-sm bg-white dark:bg-gray-900" role="table" aria-label="Product Discounts Table">
          <thead>
            <tr className="bg-gray-50 text-gray-700" role="row">
              <th className="px-4 md:px-6 py-3 text-left font-semibold" role="columnheader">{t('product_discounts.product_name')}</th>
              <th className="px-4 md:px-6 py-3 text-left font-semibold" role="columnheader">{t('product_discounts.discount_percentage')}</th>
              <th className="px-4 md:px-6 py-3 text-left font-semibold" role="columnheader">{t('product_discounts.applies_to')}</th>
              <th className="px-4 md:px-6 py-3 text-left font-semibold" role="columnheader">{t('product_discounts.start_date')}</th>
              <th className="px-4 md:px-6 py-3 text-left font-semibold" role="columnheader">{t('product_discounts.expiry_date')}</th>
              <th className="px-4 md:px-6 py-3 text-left font-semibold" role="columnheader">{t('product_discounts.status')}</th>
              <th className="px-4 md:px-6 py-3 text-left font-semibold" role="columnheader">{t('product_discounts.actions')}</th>
            </tr>
          </thead>
          <tbody role="rowgroup">
                ...
                <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                  <div>
                    ...
                    <div className="text-gray-500 text-xs">
                      {discount.product?.category?.name || t('product_discounts.no_category')}
                    </div>
                  </div>
                </td>
                <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                  {discount.discount ? `${discount.discount}%` : t('product_discounts.no_discount')}
                </td>
                <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                  <div>
                    <div className="font-medium text-gray-900">
                    ...
                    <div className="text-gray-500 text-xs">
                      {discount.client?.email || t('product_discounts.no_email')}
                    </div>
                  </div>
                </td>
                <td className="px-4 md:px-6 py-4 whitespace-nowrap">
      ...
      <div className="md:hidden space-y-4">
        {productDiscounts.map((discount) => (
          ...
          >
            ...
            {/* Client Info */}
            <div className="mb-4">
              <p className="text-xs text-theme-text-muted mb-1">{t('product_discounts.applies_to')}</p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <p className="text-xs text-theme-text-muted mb-1">{t('product_discounts.start_date')}</p>
                <p className="text-sm text-theme-text">
                  {formatDate(discount.start_date)}
                </p>
              </div>
              <div>
                <p className="text-xs text-theme-text-muted mb-1">{t('product_discounts.expiry_date')}</p>
                <p className="text-sm text-theme-text">
                  {formatDate(discount.expiry_date)}
                </p>
              </div>
      ...
      <ConfirmActionModal
        ...
        title={t('product_discounts.remove_discount')}
        message={t('product_discounts.remove_discount_confirmation')}
        confirmText={t('product_discounts.remove_discount')}
        confirmColor="red"
