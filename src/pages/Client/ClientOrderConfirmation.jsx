...
export default function ClientOrderConfirmation() {
  ...
  if (!order) return (
    <div className="flex flex-col items-center justify-center py-24">
      ...
      <p className="text-gray-500 dark:text-gray-400 mb-4">We couldn't find your order. Please check your order ID or contact support.</p>
      <Link to="/client/dashboard/orders" className="text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 font-semibold">Go to Orders</Link>
    </div>
  );
      ...
      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-gray-500 dark:text-gray-400 font-normal text-lg sm:text-xl ml-2">Thank you for your purchase</span>
          </div>
        </h1>
        <p className="text-gray-600 dark:text-gray-400 mt-2">A confirmation email has been sent. You can track your order in your dashboard.</p>
        <div className="bg-white dark:bg-gray-800 rounded-lg px-4 py-2 border border-gray-200 dark:border-gray-700 mt-4 inline-block">
              ...
              {grouped.map(group => (
                <div key={group.supplier_id} className="mb-6 bg-gray-50 dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="text-right">
                    <div className="text-xl font-bold text-primary-600 dark:text-primary-400">Supplier Subtotal: ${group.items.reduce((sum, item) => sum + parseFloat(item.total || 0), 0).toFixed(2)}</div>
                  </div>
                </div>
              ))}
              <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 mt-4">
                <div className="font-semibold mb-2 text-gray-900 dark:text-white">Shipping & Contact Information</div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-600">
                ...
                <span className="font-semibold text-gray-900 dark:text-white">${orderTotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-600">
              ...
              <div className="flex justify-between items-center py-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg px-3">
                ...
                <span className="text-xl font-bold text-primary-600 dark:text-primary-400">${finalAmount.toFixed(2)}</span>
              </div>
              <div className="space-y-3 pt-4">
                <Link to={`/client/dashboard/orders/${order.id}`} className="w-full block bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-semibold py-3 rounded-xl shadow-lg transition-all duration-200 text-center flex items-center justify-center gap-2">
