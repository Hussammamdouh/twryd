...
export default function SubscriptionRequestsStatsCards({ stats }) {
  const { t } = useLanguage();
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 lg:gap-8">
      {stats.map((stat) => (
        <div key={stat.id} className="group relative bg-white dark:bg-gray-800 rounded-2xl lg:rounded-3xl shadow-lg lg:shadow-xl border border-gray-200 dark:border-gray-700 p-6 lg:p-8 hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 overflow-hidden cursor-pointer">
          ...
          {/* Hover border effect */}
          <div className={`absolute inset-0 rounded-2xl lg:rounded-3xl border-2 border-transparent group-hover:border-opacity-50 transition-all duration-300 ${
            stat.changeType === 'positive' 
      ...
      <style jsx>{`
        @keyframes progressFill {
          from { width: 0%; }
          to { width: ${Math.abs(parseFloat(stats[0]?.change || '0')) * 2}%; }
        }
      `}</style>
    </div>
  );
