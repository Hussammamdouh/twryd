...
export default function PlanFormModal({ open, onClose, onSubmit, initialData, isEdit }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = t('plans.name_required');
    }

    if (!formData.name_ar.trim()) {
      newErrors.name_ar = t('plans.name_ar_required');
    }

    if (!formData.price_per_month || parseFloat(formData.price_per_month) < 0) {
      newErrors.price_per_month = t('plans.price_required');
    }

    if (formData.max_clients && parseInt(formData.max_clients) < 1) {
      newErrors.max_clients = t('plans.max_clients_min');
    }
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div>
                <h2 className="text-lg sm:text-xl lg:text-3xl font-bold mb-1">
                  {isEdit ? t('plans.edit') : t('plans.create')}
                </h2>
                <p className="text-blue-100 text-xs sm:text-sm lg:text-base leading-tight">
                  {isEdit ? t('plans.update_description') : t('plans.create_description')}
                </p>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 lg:space-y-8">
            ...
            <div className="bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-800 dark:to-blue-900/20 p-3 sm:p-4 lg:p-8 rounded-2xl border border-gray-200 dark:border-gray-600 shadow-sm">
              <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                ...
                <h3 className="text-base sm:text-lg lg:text-2xl font-semibold text-gray-900 dark:text-white">{t('plans.plan_details')}</h3>
              </div>
              
              <div className="space-y-4 sm:space-y-6">
                <div className="space-y-2 sm:space-y-3">
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300">
                    {t('plans.name')} <span className="text-red-500">*</span>
                  </label>
                ...
                <div className="space-y-2 sm:space-y-3">
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300">
                    {t('plans.name_ar')} <span className="text-red-500">*</span>
                  </label>
                  <div className="relative">
                    <input
                      type="text"
                ...
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white dark:bg-gray-800 hover:border-gray-400 dark:hover:border-gray-500 text-sm sm:text-base"
                    placeholder={t('plans.description_ar_placeholder')}
                    rows="3"
                    disabled={loading}
                    dir="rtl"
                  />
            ...
            <div className="bg-gradient-to-br from-gray-50 to-green-50 dark:from-gray-800 dark:to-green-900/20 p-3 sm:p-4 lg:p-8 rounded-2xl border border-gray-200 dark:border-gray-600 shadow-sm">
              <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                ...
                <h3 className="text-base sm:text-lg lg:text-2xl font-semibold text-gray-900 dark:text-white">{t('plans.plan_settings')}</h3>
              </div>
              
              <div className="space-y-4 sm:space-y-6">
                <div className="space-y-2 sm:space-y-3">
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300">
                    {t('plans.price_per_month')} <span className="text-red-500">*</span>
                  </label>
                  <div className="relative">
                    <input
                      type="number"
                ...
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder={t('plans.max_clients_placeholder')}
                      min="1"
                      disabled={loading}
                    />
                    {errors.max_clients && (
