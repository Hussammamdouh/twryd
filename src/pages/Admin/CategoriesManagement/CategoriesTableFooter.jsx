...
export default function CategoriesTableFooter({ data, totalData }) {
  const { t } = useLanguage();
  return (
    <div className="px-6 lg:px-8 py-4 border-t border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-500 dark:text-gray-400">
          {t('categories.showing', { from: data.length, to: data.length, total: totalData.length })}
        </div>
        
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <span>{t('categories.page', { current: 1, total: 1 })}</span>
